...
const dynamoTableName = "umanexus-server-centric-v3";
...
exports.handler = function(payload, context, callback) {
    ...
    if (payload.params) {
        ...
        period = {
            from: payload.params.querystring["period.from"],
            to: payload.params.querystring["period.to"]
        }

        area = {
            center: {
                latitude: Number(payload.params.querystring["area.center.latitude"]),
                longitude: Number(payload.params.querystring["area.center.longitude"])
            },
            radius: Number(payload.params.querystring["area.radius"])
        }


    }
    else {
        // Invoked from testing
